...
using HumraahFinal.Models;
using System.Collections.Generic;
using System.Data;



namespace HumraahFinal.Controllers
{
    [Authorize]
    public class AccountController : Controller
    {
        ...
        {
            var adrs = from adr in db.Addresses
                       select adr.Locality;
            var AdrLst = new List<string>();
            AdrLst.AddRange(adrs.Distinct());

            ViewBag.Locality = new SelectList(AdrLst);
            return View();
        }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> RegisterStation(RegisterStationModel model)
        {
            if (ModelState.IsValid)
            {
                /*Address adr = new Address();
                ...
                db.SaveChanges();
                */
                int AdrId = (from adr in db.Addresses
                             where adr.Locality.Equals(model.Locality)
                             select adr.id).FirstOrDefault();
